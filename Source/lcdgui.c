...
void GUI_Disbitmap(POINT Xpoint, POINT Ypoint, const unsigned char *pMap,
                   POINT Width, POINT Height, COLOR Color, bool invert)
{
...
//          GUI_DrawPoint(Ypoint + j, Xpoint + i, Color, DOT_PIXEL_DFT, DOT_STYLE_DFT);
        }
      } else {
        if ( (*(pMap + j * byteWidth + i / 8) & (128 >> (i & 7)) )) {
          GUI_DrawPoint(Xpoint + i, Ypoint + j, Color, DOT_PIXEL_DFT, DOT_STYLE_DFT);
...
//    uint16_t Cx2 = Cx1 + (2.5 * Cr), Cy2 = Cy1;
    uint16_t Cx2 = (uint16)(Cx1 + (2.5 * Cr)), Cy2 = Cy1;
...
//    DEBUG("Display String\r\n");
    GUI_DisString_EN(80, 80, "888888888", &Font48, LCD_BACKGROUND, BLUE);
