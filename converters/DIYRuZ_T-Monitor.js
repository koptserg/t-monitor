...
//          firstEndpoint.write('genTime', {time: time});
    ...
    co2: {
        ...
        type: ['attributeReport', 'readResponse'],
        convert: (model, msg, publish, options, meta) => {
//          return {co2: msg.data.measuredValue};  //not endpoint
    ...
    co2_config: {
        ...
        convert: (model, msg, publish, options, meta) => {
            const result = {};
            if (msg.data.hasOwnProperty(0xF000)) {
                result.co2_recalibration = msg.data[0xF000];
            }
            if (msg.data.hasOwnProperty(0xF001)) {
                result.co2_change = msg.data[0xF001];
            }
            if (msg.data.hasOwnProperty(0xF002)) {
                result.co2_period = msg.data[0xF002];
            }
            return result;
        },
    },
...
//            return {state: {local_time: value}};
    ...
    co2_config: {
        // set CO2
        key: ['co2_recalibration', 'co2_change', 'co2_period'],
        convertSet: async (entity, key, value, meta) => {
            value *= 1;
            if ([key] == 'co2_recalibration'){
              value = true;
            }
            const fifthEndpoint = meta.device.getEndpoint(5);
            const payloads = {
                co2_recalibration: ['msCO2', {0xF000: {value, type: 0x10}}],
                co2_change: ['msCO2', {0xF001: {value, type: 0x21}}],
                co2_period: ['msCO2', {0xF002: {value, type: 0x21}}],
            };
            await fifthEndpoint.write(payloads[key][0], payloads[key][1]);
            return {
                state: {[key]: value},
            };
        },
        convertGet: async (entity, key, meta) => {
            const fifthEndpoint = meta.device.getEndpoint(5);
            const payloads = {
                co2_recalibration: ['msCO2', 0xF000],
                co2_change: ['msCO2', 0xF001],
                co2_change: ['msCO2', 0xF002],
            };
            await fifthEndpoint.read(payloads[key][0], [payloads[key][1]]);
        },
    },
...
const device = {
        zigbeeModel: ['DIYRuZ_T-Monitor'],
        model: 'DIYRuZ_T-Monitor',
        vendor: 'DIYRuZ',
        description: '[T-Monitor sensor](https://github.com/koptserg/t-monitor)',
        supports: 'temperature, humidity, illuminance, e-ink, pressure, battery, occupancy, time, co2',
        fromZigbee: [
            fz.config_epd,
            fz.local_time,
            fz.battery_config,
        ...
        configure: async (device, coordinatorEndpoint) => {
            ...
            await bind(fifthEndpoint, coordinatorEndpoint, [
                'msCO2',
            ]);
...
//            await fifthEndpoint.configureReporting('msCO2', msBindPayload);
        ...
        exposes: [
            ...
            exposes.numeric('co2', ACCESS_STATE).withUnit('ppm').withDescription('Measured CO2 in ppm SCD4x'),
            exposes.binary('occupancy', ACCESS_STATE).withDescription('Indicates whether the device detected occupancy'),
            exposes.enum('co2_recalibration', ACCESS_STATE | ACCESS_WRITE, ['perform']).withDescription('Perform forced recalibration.'),
            ...
            exposes.numeric('co2_change', ACCESS_STATE | ACCESS_WRITE | ACCESS_READ).withUnit('ppm').withDescription('CO2 SCD4x min absolute change (default = 10 ppm)'),
            exposes.numeric('co2_period', ACCESS_STATE | ACCESS_WRITE | ACCESS_READ).withUnit('sec').withDescription('CO2 SCD4x measurement period (default = 10 sec)'),
